 ...
 *    改完温度后又测三次，仍是 4 / 6 / 2 —— 但那仍是同一批体育稿，
 *    **只能说明「模型对模板化体育标题判断不稳」，不能说明生产里不稳**。
 *
 *    所以：温度 0 保留（第 1 层理由成立），但**不要引用任何「不稳定的倍数」**
 *    当论据 —— 那个数字来自不具代表性的输入。体检接口已改为按生产口径收窄
 *    （见 `dedupe-check` 的 `llmJudgeParams.scope`），要下结论请用改后的口径重测。
 *
 * 另：温度 0 只是**必要条件**，不是充分条件。降级链会按顺序换通道
 * （`zhipu` → `zhipu-flash` → `deepseek`），**不同型号给出不同答案**也是随机源之一，
 * 而当前体检并不报「这次是哪个通道答的」。要做稳定性的正式结论，先把这个补上。
 */
