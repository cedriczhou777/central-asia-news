...
export interface DedupResult<T> {
  ...
  llm: {
     ...
     *
     * ⚠️ 它是**排除性工具**，而且已经用掉了一次：2026-09-22 加字段后实测，
     * **通道不是原因**（两边证据见 `JUDGE_TEMPERATURE` 的第 2 层说明：
     * 通道变了结论没变、通道没变结论却在变）。所以别看到这个字段就以为
     * 「换个通道能修」—— 钉固定通道解决不了判定不稳。
     *
 ...
 *    判定类调用沿用它就是在给结论注入随机性。
 *
 * 2. **实测确实不稳定，且已排除「换通道」这个嫌疑**（2026-09-22 线上实测）：
 *
 *    **结论：通道不是原因。** 同样输入连跑，模型的判定会变，而变的时候
 *    答题的通道并不跟着变。两条互相独立的证据：
 *
 *    (a) **通道变了、结论没变**：2 天窗口连跑 10 轮（5 国 × 10 = 50 次判定），
 *        tj / kz / uz 的通道在 `zhipu` 与 `zhipu-flash` 之间来回换，
 *        而**同一批候选对两条通道都判过、结论逐次完全一致**（0 分歧）。
 *    (b) **通道没变、结论变了**：7 天窗口连跑 5 轮，uz 的通道 5 次全是
 *        `zhipu-flash`，仍出现 3 / 4 / 4 / 4 / 3 的分歧；tj 同样是 `zhipu-flash`，
 *        两个候选对**各自都在翻**（`是/是/否/是/是`、`否/是/是/否/是`）。
 *
 *    ⇒ **给判定钉一条固定通道解决不了这个问题**，不要去改 `PROVIDERS` 的顺序。
 *
 *    翻转的对长什么样（是边界案例，不是随机崩坏）：uz 翻转那对是
 *    「总统女儿呼吁投资新塔什干」的两种写法（3 是 / 2 否）；
 *    tj 翻转的一对是「上合反垄断机构会议 vs 上合经贸部长会议」——
 *    看着是**两个不同的会**，模型 4 / 5 次判「是」，即**多数时候会误合并**。
 *    失效还是双向的：az 的 9 个候选对 5 轮全稳，却把好几对明显的同事件
 *    （音乐日比赛、航空大奖、石油工人授勋）**一致地判成「不是」**（漏合并）。
 *
 *    ⚠️ **测稳定性必须喂够候选对，否则会得出「稳定」的假结论。**
 *    同一套代码、同一份线上数据：2 天窗口（每国 1–3 对、共 11 对）连跑 10 轮
 *    零分歧；换 7 天窗口（每国 0–9 对）第 2 轮就出分歧。
 *    候选对只有 1 个时「稳定」几乎是必然的 —— 那不是稳定，是**样本没有检验力**。
 *    所以看稳定性报告先看 `candidatePairs`，个位数的结论不要采信。
 *
 *    温度 0 保留：第 1 层理由（下游是删除动作，要同输入同输出）成立，
 *    但它显然**不足以保证确定性**。当前结论：`SAME_EVENT_JUDGE` 保持关闭。
 */
