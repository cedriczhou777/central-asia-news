 ...
 *    改这里时记得核对那边。
 *
 * 2. **人名保留拉丁字母，不音译成汉字**（见提示词第 6 条）。
 *    中亚/高加索人名的汉字音译各家不统一、常常不准，投资者按拉丁写法反而检索得到。
 *
 *    ⚠️ 这条会**拉低标题的汉字占比**，而 `isChineseText`（阈值 0.4，拉丁字母计入分母）
 *    是「翻译是否成功」和「能不能推送」的判据。2026-09-22 用线上 400 篇量过余量：
 *    标题汉字占比 min=0.405 / p1=0.464 / 中位=0.931（0 篇低于 0.4），
 *    所以正常标题加一两个人名不会掉下去 —— 但**余量只有 0.005 的那几条是贴线的**。
 *    改动上线后要复查这个分布，别让「保留人名」变成静默丢稿。
 *    （同时验证过一个「只按字母算、把数字排除出分母」的替代口径，
 *    最小值只从 0.405 提到 0.410，收益太小、不值得动一个被 4 处调用的判据，故未改。）
 *
 * 设计要点
 * --------
...
6. **人名写法（重要）**：人名**一律保留拉丁字母**（该人名的通行英文转写），**不要**音译成汉字。
   例：Токаев → Tokayev；Мирзиёев → Mirziyoyev；Әлиев → Aliyev；Раҳмон → Rahmon。
   原文用西里尔字母时，按**通行英文转写**写（俄语 -ov/-ev/-in 体系；哈萨克语 -uly/-kyzy 或 -ov 体系，
   取最常见写法），**不要自己按字母硬拼**。
   ⚠️ 这条**只针对人名**。机构名、公司名、地名、职务照常译成中文
   （本来就以拉丁字母通行的机构名如 KEGOC / AZAL / EBRD 保留拉丁即可）。
