...
// （2026-09-24 早报由 08:00 提前到 07:00；下面「两段窗口必须同步调」记了连带改动。）
//
// 回看窗口怎么切（改动的原因）：
// 旧版两次都用「过去 24 小时」，两段窗口中间有 13 小时重叠，结果是
//   1) 同一条新闻连着进两次推送；
//   2) 草稿标题里的日期取的是 UTC 日期，两次的北京时刻落在同一个 UTC 日，
//      两次生成的 5 国草稿标题完全相同，草稿箱里成对出现。
// 现在改成首尾相接、互不重叠的两段：
//   早报 07:00 → 回看 12 小时（昨日 19:00 → 今日 07:00）
//   晚报 19:00 → 回看 12 小时（今日 07:00 → 今日 19:00）
// 加起来正好覆盖完整的一天，既不重复也不漏。
//
// ⚠️ **两段窗口是绑在一起的：改一个时段必须同步改两处 hours。**
// 窗口起点算的是 `now - hours`（在 `push/route.ts` 里），与触发时刻是浮动关系，所以：
//   - 早报往前挪 1 小时 ⇒ 早报 hours 要**减** 1（08:00/13h → 07:00/12h），
//     否则起点会滑到昨日 18:00，与前一晚的晚报**重叠 1 小时**（重复推送）；
//   - 同时晚报 hours 要**加** 1（11h → 12h），否则 [今日07:00, 今日08:00]
//     这 1 小时**没有任何一轮覆盖**，那一小时发的新闻会永久丢掉。
...
// hours 由定时表给出：早报 12h、晚报 12h，两段首尾相接不重叠。
