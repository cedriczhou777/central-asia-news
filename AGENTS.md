...
- **门 1 —— ⚠️ 金标集在仓库里并不存在，这一门目前跑不了。**
  文字引用在（第 16 条的 IDF 实验标过 **42 对 S / 24 对 D**），但**没有落盘成文件**
  （`grep` 全仓只有 AGENTS.md 里的叙述，没有数据）。要跑门 1 必须先把它固化下来，
  否则每换一个人就要重标一次。验收标准仍然是 **D 一个都不许判「是」**（误合并不可逆 = 丢信息）；
  S 允许有缺（漏合并只是留下重复，看得见）。
- **门 2 —— 需要一条「可指定语料」的探针。** 现有 `dedupe-check?llm=1` 只能按 `days`+`limit`
  取**id 升序的前 N 条**，没法喂指定的对（金标集里的对可能早已不在窗口里）。
  做门 1/门 2 之前要先补一个「按 id / 按标题喂进去判」的入口。
- **门 3 —— 影子运行：2026-09-24 已开始，结论见上面那段「重要修正」。**
  另外记一条：**先看 `llmJudgeParams.judgePromptVersion`**（本次新增）。
  提示词改动在响应里本来完全不可见，版本号对不上会把 A/B 的结论读反。
- **判据校准（2026-09-24 已做，`JUDGE_PROMPT_VERSION = 2`）**：给 `buildPairPrompt` **补正例**
  ——① 「同一家公司同一个产品被多家媒体换词重写」＝同一件事（Unibank 那条原样写进去）；
  ② 「其中一条标题有笔误/重复字」也要判是；并把「否认某报道 vs 该报道本身」加进反例。
  注意**没有删掉「拿不准就判否」** —— 那条对误合并（不可逆）依然必要，缺的是正例校准。
  改完必须重新跑影子运行复测（`?days=1&llm=1&debug=1`），确认这三条错判翻转且没有新的误合并。
