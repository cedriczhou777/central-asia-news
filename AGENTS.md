...
- **内容级去重**：统一入口是 `src/lib/same-event.ts` 的 `dedupeStories()`，**入库端与推送端共用同一个**（不要各写一套）。三层判据：
  ...
  3. `llm_same_event` —— 模型判「表述不同、实际是同一件事」，每国 1 次调用，走 `translate.ts` 的免费优先降级链；不可用时自动降级为只做 1、2。
     ...
     ⚠️ **实测结论（2026-09-22）：判定确实不稳，但原因不是通道 —— 别去动 `PROVIDERS`。** 两条互相独立的线上证据（都走生产口径）：
     - **通道变了、结论没变**：2 天窗口连跑 10 轮（5 国 × 10 = 50 次判定），tj / kz / uz 的通道在 `zhipu` 与 `zhipu-flash` 间来回换，而同一批候选对**两条通道都判过、结论逐次完全一致**（0 分歧）。
     - **通道没变、结论变了**：7 天窗口连跑 5 轮，uz 的通道 5 次全是 `zhipu-flash`，仍出现 `3 / 4 / 4 / 4 / 3` 的分歧；tj 同样是 `zhipu-flash`，两个候选对各自都在翻（`是/是/否/是/是`、`否/是/是/否/是`）。
     ⇒ **钉一条固定通道解决不了**。随机源在模型/服务端侧。
     翻转的是**边界案例**（uz 翻转那对是「总统女儿呼吁投资新塔什干」的两种写法，3 是 / 2 否）；tj 翻转的一对看着是**两个不同的会**（上合反垄断机构会议 vs 上合经贸部长会议），模型却 4 / 5 次判「是」—— 即**多数时候会误合并**。失效是双向的：az 的 9 个候选对 5 轮全稳，却把好几对明显同事件一致判成「不是」（漏合并）。
     ⚠️ **测稳定性必须喂够候选对，否则会得出「稳定」的假结论。** 同一套代码、同一份线上数据：2 天窗口（每国 1–3 对、共 11 对）连跑 10 轮**零分歧**；换 7 天窗口（每国 0–9 对）**第 2 轮就出分歧**。候选对只有 1 个时「稳定」几乎是必然的 —— 那不是稳定，是样本没有检验力。**看稳定性报告先看 `candidatePairs`，个位数不要采信。**
     体检响应带 `provider` 字段（`DedupResult.llm.provider`，pair / group 两种形态都带）—— 它的价值是**排除性**的：把「换通道」这一嫌疑一次性排掉，别指望它「修」什么。候选对为 0 时该字段为空（没真调用模型），不是 bug。
