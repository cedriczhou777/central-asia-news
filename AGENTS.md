...
2. **闸 3（同一件事）的召回门槛要不要改造（产品决策）** —— 2026-09-23 用户报「今天早上哈萨克还是出现了
   几乎完全相同的新闻」，实测确认 09-22 的 kz 有**三篇**同一件事（阿斯塔纳机场 iPhone 走私）全部正常入库。
   量出来的两个缺口：**① 门槛 0.35 的召回不行且与语种无关**（同语种 ru、同事件、都写着 223 的两条只有 **0.247**，
   候选阶段就被丢、模型根本不会被问 ⇒ **光打开 `SAME_EVENT_JUDGE` 修不了**）；
   **② 闸 3 只在同一批内跑，跨轮次没有任何同事件判据**。
   数据、复现方法、为什么「先看召回再谈开关」见「抓取放宽与内容去重」一节。
   ⚠️ 任何改动都要先过「误合并不可逆」这一关（误合并 = 一次性删掉多条不同新闻），
   并留独立一天的验证（与 B-3 同一个纪律）。
...
12. **2026-09-23 晚 19:00 那轮，是 intl 改判（`9e82d33`）的第一次真实入库执行。**
...
- **内容级去重**：统一入口是 `src/lib/same-event.ts` 的 `dedupeStories()`，**入库端与推送端共用同一个**（不要各写一套）。三层判据：
  ...
  3. `llm_same_event` —— 模型判「表述不同、实际是同一件事」，每国 1 次调用，走 `translate.ts` 的免费优先降级链；不可用时自动降级为只做 1、2。
     ...
     **用项目自己的 `similarity()`（= 候选门槛用的那个函数）实测**：
     ...
     | 对比 | 语种 | similarity | 过 0.35？ |
     |---|---|---|---|
     | Newtimes.kz ↔ Total.kz（同事件、都写着 223） | **ru ↔ ru** | **0.247** | ❌ |
     | 3950 ↔ 3941（**中文译文**标题） | zh ↔ zh | 0.438 | ✅ |
     | 3941 ↔ 3920（中文译文标题） | zh ↔ zh | 0.350 | 勉强 ✅ |
     | 3920 ↔ 3950（中文译文标题） | zh ↔ zh | 0.302 | ❌ |
     | Egemen Qazaqstan ↔ ru 源（**另一条**哈萨克语稿） | kk ↔ ru | 0.125 / 0.143 | ❌ |

     ⇒ 两个**独立**缺口，别混为一谈：
     - **缺口 1：门槛的召回不行，而且这次跟语种没关系。** 三篇**全是俄语源**、同一件事、都写着 223，
       实测 Newtimes ↔ Total 也只有 **0.247 < 0.35** ⇒ 在候选对阶段就被丢掉，**模型根本不会被问**。
       所以「把 `SAME_EVENT_JUDGE` 打开」**修不了这个案例** —— 开关管的是「已召回的对判得对不对」，
       管不了「先召不召得到」。**先看召回，再谈开关。**
       （`similarity` 是**字符 bigram Jaccard**，对「换词重写、语序不同」的短标题天然低分，
        不加语种也照样低；中文译文之所以分高，是因为同一件事的译文会在同一处收敛到同一批词。
        跨语种只是**更差**的一个特例 —— 上面 kk ↔ ru 那行是同一个故事被哈萨克语媒体也报了一遍，
        实测 0.125–0.143，连边都摸不到。）
     - **缺口 2：闸 3 只在「同一批」内跑，跨轮次完全没有同事件判据。** `dedupeStories(list)` 的入参是
       ...
       （本例三篇的 `publishedAt` 落在 09-22 15:00–17:12Z（= 北京 23:00–01:12），
        `id` 也挨得近、像同一批；但接口不返回 `createdAt`，**没能钉死**具体是哪几轮 —— 别写成结论。）
     ...
     📌 **复现方法**：`GET /api/articles` **不返回 `original_title`**，原文标题只能回源站取。
     写个临时 tsx 探针，`import { fetchFeed } from '../src/lib/feed-fetch'` +
     `import { similarity } from '../src/lib/utils'`，拉该国的几个源、按关键词捞出条目，
     再用项目自己的 `similarity()` 两两算。⚠️ **别自己另写一个相似度实现** —— 那样测的就不是线上跑的东西
     （与上面「体检接口的输入必须与 push 同口径」是同一条纪律）。
     2026-09-23 是用这个方法量的（探针用完即删，**没留在仓库里**）。
