...
#### 第一步（零模型、可回滚、可立刻做）：把 `same_title` 的判据接到闸 2，封住「跨轮重复」

- **为什么排第一**：AIIB 那三条横跨两轮（`id=4192` 在 4175–4221 轮，`4245|4247` 在 4227–4250 轮），
  `kz 3741|3915`（无人驾驶出租车）差 174 个 id —— 这类**批内判据在结构上不可能接住**。
- **做法**：闸 2 除了「链接集合 + 原文指纹集合」再查一个「近窗口的**中译标题**集合」，
  用与 `isSameTitle` **同一个** `TITLE_IDENTICAL_MIN_SIM` 做近逐字比较。
  `getArticleIdentities` 已经在选 `title` 了，查询不用新写。
- **两个必须注意的点**：
  1. 时间轴要用 **`created_at`（入库时间）而不是 `published_at`** —— 要回答的问题是
     「我最近是不是已经推过这条」，这正是 B-3 记的 `published_at` 口径问题。
  2. 闸 2 是**入库前**的闸，丢在这里的行**库里没有、事后不可追**
     ⇒ 必须用**紧**判据（≥0.95 逐字级），不能像闸 3 那样放宽。
     **宁可留重复（看得见），不要丢稿（看不见）** —— 与 `MIN_HAN_*` 那次的取舍同向。
- **收益**：截掉「同一条新闻换源/换语种、隔轮到达」这一类。零 token。
...
| `pnpm test:dedup` | 内容去重双向语料（113 条） | 否 |
