...
19. **早报窗口的起点用了 `now - hours`，而 `now` 是「推送真正开始跑的时刻」**。
    ...
    **① 昨日 19:00–21:30 的覆盖直接丢了；② 窗口内只剩「刚跑完那一轮」的稿子。**
    ⇒ 改法：窗口起点取「上一轮推送的终点」（早报 = 昨日 19:00，晚报 = 今日 07:00），
    而不是从执行时刻往回数。
...
**19:00 那轮要看的（缺陷 18 的线上证据）**：`GET /api/fetch-news` → `lastRun.summary.dedup.window`
- `rows` 从 **1000** 涨到真实值（当时窗口 1228 行，19:00 会更多）⇒ 分页生效。
- **仍然卡在 1000** ⇒ 分页没生效或又坏了，立刻查。
- 顺带看 `dedup.titleDrops`（闸 2 之二的逐条明细）与 `againstDbTitles`：
  本轮 `againstDbTitles = 0`、`titleDrops = []`，属正常（跨轮近逐字重复本来就少）；
