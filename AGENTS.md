...
- **内容级去重**：统一入口是 `src/lib/same-event.ts` 的 `dedupeStories()`，**入库端与推送端共用同一个**（不要各写一套）。三层判据：
  ...
  3. `llm_same_event` —— 模型判「表述不同、实际是同一件事」，每国 1 次调用，走 `translate.ts` 的免费优先降级链；不可用时自动降级为只做 1、2。
     ...
     ⚠️ **判定必须 `temperature: 0`**（`same-event.ts` 的 `JUDGE_TEMPERATURE`）。`translate.ts` 的默认温度是 0.3 —— 那是给**翻译**留用词变化的，判定类调用沿用它就是给结论注入随机性。**但这条只是设计上的必要条件，不要引用任何「不稳定的倍数」当论据**：2026-09-21 测到的「同一份候选对两次判 4 对 / 10 对」来自一个**口径不对**的体检（见下条），那批候选对全是体育新闻，不能代表生产。温度 0 保留。
     ⚠️ **按生产口径重测后，「不稳」依然成立 —— 所以别把温度 0 当成已解决问题。** 改口径后连测三次，结论仍逐次不同：**az 判 1 / 0 / 1 对、uz 1 / 1 / 0 对**（kg / kz / tj 三次一致）。只有三个样本，**没有可信的倍数**，但这个现象不是「输入不对」造成的。**下一个嫌疑是通道切换**：三次耗时 4.7s / 11.7s / 68.7s（差 15 倍），而降级链是 `zhipu → zhipu-flash → deepseek`、不同型号对同一输入可以给出不同答案。体检响应自 2026-09-22 起带 `provider` 字段（`DedupResult.llm.provider`，pair / group 两种形态都带）—— 连跑两次结论不同时，**先看 provider**：通道不同 = 可修（给判定钉一条固定通道），通道相同 = 模型/服务端本身不稳，改不了。**在分类完成前，`SAME_EVENT_JUDGE` 保持关闭。**
