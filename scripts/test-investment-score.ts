...
// ============================================================
// 二之二、闸门的**语言偏置**回归（西里尔 / 中亚民族语言）
// ============================================================

/**
 * ## 这一节钉的是什么
 *
 * `isInvestmentTopic` 跑在**原文**上（翻译刻意排在闸门之后，为了省翻译钱），
 * 而 2026-09-22 之前它的词表是**纯英文 + 纯中文** —— 于是它实际在按语言筛稿：
 * 实测西里尔条目通过率 4.1%、拉丁条目 82.6%，**差 20 倍**。
 * 生产上的表现是 `kz` 六个源里只有英文的 The Astana Times 稳定供货，
 * 塔吉克三个源合计 30 条、**候选 0 条**。
 *
 * ## 语料全部是真实标题（不是编的）
 *
 * 下面每条都来自当日各源 feed 的实测（`pnpm analyze:source-language kz --dump`）。
 * 双向组织，两边都不能少：
 *   - **必须放行**：投资/外资/物价/工资/预算/基建/能源 —— 这些此前全被扔掉。
 *   - **必须挡掉**：地震、壁画、车祸、爆炸、天气、体育、书展、判决 —— 闸门不能变成摆设。
 *
 * ⚠️ **输入与生产一致**：闸门吃的是 `` `${title} ${contentSnippet}` ``，所以下面的
 * 语料也是「标题（+ 描述）」。大部分条目的信号就在标题里，只给标题即可；
 * **个别稿子的信号在描述里**（例如 Stadler 那条靠描述里的 `көлік`），
 * 那几条把真实描述一起给 —— 这正是「只看标题会误判」的实例，不能省。
 * 全部描述文字都是当日实测原文。
 */
{
  section('闸门语言覆盖：西里尔/中亚语言（真实标题双向语料）');

  // ----- A. 必须放行 -----
  // `[标题, 描述?, 这句稿子此前为什么进不来]`
  const CYR_KEEP: Array<[string, string, string]> = [
    ['Stadler вагондары Қазақстанда қашан қатынай бастайды?',
      'Қазақстанда жаңа Stadler вагондарымен жабдықталған пойыздар қашан жолға шығатыны белгілі болды. Көлік вице-министрі Жәнібек Тайжановтың айтуынша, алғашқы вагондарды жолаушылар тасымалына қазан айының',
      '铁路车辆采购（信号在描述里）'],
    ['Зейнетақы жинақтарына берілетін мемлекеттік кепілдік: 2027 жылдан бастап не өзгереді?', '',
      '养老金国家担保（民生）'],
    ['Пәтер жалдау тағы қымбаттады: Баға қай өңірлерде өсті?', '', '房租上涨（物价/民生）'],
    ['«Qarmet»: өндірістің тарихи қадамы – өңірдің дамуына тың серпін', '',
      '钢铁企业产能里程碑（产业）'],
    ['Көмір бағасы көңіл көншітпейді', '', '煤价（能源/物价）'],
    ['Пенсии казахстанских полицейских не перестанут индексироваться', '', '养老金指数化（民生）'],
    ['В Астане реализуют три проекта для разгрузки городских магистралей', '', '城市道路项目（基建）'],
    ['Земли стоимостью свыше 112 миллионов теңге вернули государству', '', '土地收回（资产）'],
    ['Когда дадут тепло в Казахстане — Минэнерго', '', '供暖 + 能源部（公用事业/能源）'],
    ['Капельное орошение вне закона: фермеры Экибастуза заплатили 8 миллионов теңге за забор воды', '',
      '灌溉与罚款（农业/监管）'],
    ['В Казахстане самые дешевые дороги — глава «КазАвтоЖол» о тарифах', '', '公路收费（基建/价格）'],
    ['Китайская Shandong Shanzuo инвестирует до $100 миллионов в создание промзоны в Оше', '',
      '中国外资建工业园（外资）'],
    ['Акимат Алматы выделил средства на ремонт дорог', '', '地方政府拨款（政经）'],
    // 乌兹别克语（西里尔）—— UzA 是乌兹别克语源，不是俄语源，实测确认
    ['Тадбиркор учун яна бир қулай имконият', '', '乌兹别克语「企业家」（工商）'],
    ['Гаровсиз 200 миллион сўм кредит ва 300 миллион сўмлик грант Бундай имконият кимлар учун? (+видео)', '',
      '乌兹别克语「无抵押贷款 + 补贴」（融资）'],
    ['Тўрақўрғон хурмолари Европага экспорт қилинмоқда', '', '乌兹别克语「出口」（外贸）'],
  ];
  for (const [title, desc, why] of CYR_KEEP) {
    const text = `${title} ${desc}`;
    ok(`放行：${title.slice(0, 32)}…`, isInvestmentTopic(text) === true, `被挡住了（${why}）`);
  }

  // ----- B. 必须挡掉 -----
  // 描述也是实测原文：这样「挡掉」这个结论才是被真实输入支持的，
  // 而不是「标题里恰好没有词」这种弱结论。
  const CYR_DROP: Array<[string, string, string]> = [
    ['Каспий теңізінде жер сілкінісі тіркелді',
      'Каспий теңізінің акваториясында магнитудасы 4,1 болатын жер сілкінісі тіркелді.', '地震'],
    ['Астанада жаңа мурал пайда болды',
      'Астананың Алматы ауданындағы Абылай хан даңғылы, 55-мекенжайда орналасқан тұрғын үйдің қасбетін жаңа мурал безендірді.', '壁画'],
    ['Петропавлда мотоцикл жүргізушісі 38 жастағы ер адамды қағып кетті',
      'Ер адам оқиға орнында көз жұмды. Ал 41 жастағы жүргізуші ауыр жарақатпен ауруханаға жатқызылды.', '车祸'],
    ['Йеменнің Таиз қаласында жарылыс болды',
      'Йеменнің оңтүстік-батысындағы Таиз қаласының Зу-Баб ауданындағы базарда жарылыс болды.', '他国爆炸'],
    ['Дожди с грозами прогнозируют на большей части Казахстана',
      'Какой будет погода в ближайшее время, рассказали синоптики.', '天气预报'],
    ['«Услышали рев медведя»: туристы в районе «Медеу» вызвали спасателей',
      'Спасатели оказали помощь четырем молодым туристам в горах Алматы.', '旅游趣闻'],
    ['Жағажай волейболынан Қазақстан құрамасы тағы да жеңіске жетті',
      'Жазғы Азия ойындарында жағажай волейболынан Қазақстан ерлер командасы кезекті ойынын өткізді.', '体育'],
    ['Азиада-2026: Қазақстан кешенді жүзуден эстафетада финалға шықты',
      'ХХ жазғы Азия ойындарында ерлер арасында 4×100 метрге кешенді жүзу эстафетасынан іріктеу кезеңі өтті.', '体育'],
    ['Кітап оқу адамның ойлау қабілетіне қалай әсер етеді?',
      'Кітаптың адам өміріндегі орны мен оның ойлау қабілетіне әсері туралы пікірлер жиі айтылып жүр.', '读书/生活方式'],
    ['Абу-Дабидегі кітап көрмесі',
      'Қазақстан әлемдік кітап алаңындағы маңызды жиындардың бірі – Абу-Дабидегі 35-халықаралық кітап көрмесіне қатысты.', '书展/文化'],
    ['Приговор Гульнаре Бажкеновой оставлен без изменений',
      'Городской суд Алматы рассмотрел апелляционную жалобу бывшего главного редактора издания Orda.kz.', '刑事判决'],
    ['Дефицит полицейских в Казахстане достиг 20 тысяч человек',
      'Дефицит полицейских к настоящему моменту еще больше.', '警力缺口'],
  ];
  for (const [title, desc, why] of CYR_DROP) {
    ok(
      `挡掉：${title.slice(0, 32)}…`,
      isInvestmentTopic(`${title} ${desc}`) === false,
      `被误放行了（${why}）`,
    );
  }
  ...
  // ----- C. 误命中护栏 -----
  //
  // 这几条是**我自己在词表第一版里写错**、又被逐词体检抓出来的。
  // 它们全是「常用词里恰好包含了某个词干」，一旦有人把坏词干加回来，
  // 这几条会立刻红 —— 而不是等到翻译账单涨上去才发现。
  const COLLISION_GUARDS: Array<[string, string, string]> = [
    ['Потребность в сотрудниках растёт', 'сотрудник（员工）', '写 `рудник` 会让所有「员工」新闻过关'],
    ['Каким образом это работает', 'каким（什么样的）', '写 `аким` 会让所有含「каким」的句子过关'],
    ['Охрана труда и техника безопасности', 'труда（劳动）', '写 `руда`/`руды` 会让所有「劳动」新闻过关'],
    ['Баары жакшы, рахмат', 'баары（全部，吉尔吉斯语）', '写 `баа` 会让 Kabar 整源过关 = 闸门失效'],
  ];
  for (const [text, collision, harm] of COLLISION_GUARDS) {
    ok(
      `护栏：不含投资词时「${collision}」不能过关`,
      isInvestmentTopic(text) === false,
      harm,
    );
  }
}
