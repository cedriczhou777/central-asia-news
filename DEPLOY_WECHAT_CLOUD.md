...
> 取舍是刻意的：**宁可草稿晚一点（07:00 触发 → 最晚 09:30 出），也不要在库半空时推。**
...
即 **07:00 那轮的草稿约 08:25 出现、19:00 那轮约 20:25**（修之前是 09:40 / 20:40）。
...
> 2026-09-24 那轮 185 分钟，原因是上游 429 重试），再加推送 ~10 分钟，
> 所以**草稿实际出现时间在 08:25 / 20:25 左右**（慢轮更晚），而不是 07:00 整。
> 这段延迟是设计内的，别当成故障。
>
> ⚠️ 触发时刻和回看窗口是绑的：改时刻必须同步改 `scheduler.ts` 里两处 `hours`，
> 以及 `container.config.json` 的 `warmup-morning` 预热时间。
>
...
启动定时任务调度器...
已注册公众号推送任务：0 7 * * * (早上 07:00（早报）)，回看 12 小时
已注册公众号推送任务：0 19 * * * (晚上 19:00（晚报）)，回看 12 小时
共注册 2 个定时任务
```

- 看到「共注册 **2** 个定时任务」= 版本对了。**如果是 4 个，说明部署的还是旧代码。**
- 看到两行都是「回看 **12** 小时」= 代码里「两次推送窗口首尾相接不重叠」的设计已生效（2026-09-24 早报提前到 07:00 后为 12h + 12h）。
...
> 适合人工补跑；日常两次定时推送由应用内调度器按 12h / 12h 的增量窗口跑。
...
上面都通了，再等下一个 07:00 / 19:00 看是否自动触发。日志关键词：

```
触发公众号推送任务 (早上 07:00（早报）)
开始抓取当天新闻...
新闻抓取已触发: {"success":true,...}
新闻抓取完成: {"saved":40,"sourceCounts":{...}}
开始执行微信公众号推送任务（时段 morning，回看过去 12 小时）...
微信公众号推送：2026-09-18 早报，汇总 ...T19:00:00.000Z 至 ...T23:00:00.000Z（过去 12 小时）
...
本项目的两次定时（北京 **07:00** / 19:00 = UTC **23:00**（前一日）/ 11:00）**触发时刻都落在非高峰**，按半价计。
⚠️ 但**一轮翻译要跑 75–185 分钟**，所以后半程会滑进高峰窗（01:00–04:00 UTC）：
07:00 触发跑满 185 分钟 → 到 02:05 UTC，约 65 分钟落在高峰；旧口径 08:00 触发同样跑满 → 到 03:05 UTC，约 125 分钟落在高峰。
**提前到 07:00 反而减少了高峰段的占比。** 真要省钱得先治 429（见 AGENTS.md 缺陷 21）。
