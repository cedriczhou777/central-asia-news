...
**账号/控制台侧没有任何可做的了**，三个必要条件本来就齐：
Key 已配、`glm-4.7-flash` 是**单价 0 元的永久免费模型**（不消耗资源包，所以「资源包到没到账」与它无关）、
`thinking` 已正确关闭、串行调用符合免费档的并发限制。
剩下的唯一瓶颈是 **1305 平台服务过载** —— 智谱自己的负载，配不了（实测 122ms 就快速失败）。
想靠[权益等级](https://bigmodel.cn/usercenter/equity-mgmt/user-rights)提并发也走不通：
积分只能靠**花现金**获得（1 元 = 1 积分），V1 要 2000 积分、V2 要 10000，且**消耗资源包/赠金不计积分** ——
对本项目用量完全不划算。
...
按每轮约 175 篇走付费估：**≈ $0.26/轮 ≈ ¥1.9/轮 → 两次/天 ≈ ¥3.8/天 ≈ ¥115/月**。

**要真正"全免费"，只有两条路（都得写代码/接资源，不是配一配就好）：**

| 方案 | 做法 | 效果 | 代价 |
|---|---|---|---|
| **A. 加第二个免费通道**（推荐） | 在付费通道**之前**再插一家免费厂商，例如魔搭 ModelScope（每用户 2000 次/天免费，Qwen 系列；本项目每天约 600 篇，够用）或硅基流动 | 智谱忙时由它顶上，**依然免费且不丢内容**，能吃掉大部分付费量 | 需注册+实名拿 Key；接进 `PROVIDERS` 后要**先验证它 JSON 输出稳定**（本项目要求严格 JSON + 分类枚举） |
| **B. 失败的留到后面再免费重试** | 现在逻辑是「这篇此刻翻不了 → 立刻花钱翻」；改成「先挂起，本轮结束后隔一段时间用免费通道重试，实在不行再付费」 | 免费命中率最大化 | 需要一个「待翻译队列」（DB 加状态），改动面比 A 大；草稿可能晚一点 |
